<script>
								$(document).ready(function(){
									...
									$("table tr td a#penagihanun").click(function(){
											var obj=$(this);
											$.ajax({
													type: "POST",
													data: '<?php echo $this->security->get_csrf_token_name();?>=<?php echo $this->security->get_csrf_hash(); ?>&',
													...
													success: function(msg) {
														$("#waittugas22").remove();
														if(msg==1){
															$(obj).parent('td').css('background-color','none');
															$(obj).css('color','white');
														}else{
															$(obj).parent('td').css('background-color','red');
															$(obj).css('color','white');
...
<div class="one_full">
	...
	<section class="item">
		<div class="content">
		<style>
		table#ddc tr:hover{background-color:#888888;}
		</style>
				<table cellspacing="0" id="ddc">
					<thead>
						<tr>
						...
						?> 
							<tr>
								...
								if($lunas!=""){
									...
									// echo $lunas;
									echo "<a id='penagihanun' style='color:black' href='".base_url('admin/pam/laporan_penagihanunproses/'.$bulannya['id_pelanggan'].'/'.$bulannya['id'].'')."' >".$bulannya['harus_bayar']."-".$lunas."</a>";
								}else{
